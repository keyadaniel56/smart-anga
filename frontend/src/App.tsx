...
// Optional: Keep these if you plan to use them in your JSX, or remove them to clear warnings:
import { RiskBadge } from './components/ui/RiskBadge';
import { RiskDial } from './components/ui/RiskDial';
import { EmptyState } from './components/ui/EmptyState';

// Ensure this file exists at src/context/LanguageContext.tsx, 
// or update the path to point to your actual context file.
          ...
          <main className="flex-1 p-4 sm:p-6 lg:p-7 min-w-0 overflow-x-hidden">
            {activeTab === 'overview_gis' && (
              <div className="space-y-6 animate-in fade-in duration-200">
                ...
                {/* Catchment Context Card with RiskBadge and RiskDial */}
                <div className="bg-white/80 border border-sand-200 rounded-3xl p-5 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {/* Using RiskBadge here */}
                      <RiskBadge level={currentLocation.vulnerabilityIndex > 70 ? 'critical' : 'moderate'} />
                  ...
                  {/* Using RiskDial here for a quick visual metric */}
                  <div className="flex items-center gap-4 bg-sand-50/80 p-3 rounded-2xl border border-sand-200 self-start md:self-auto">
                    <div className="w-16 h-16 flex-shrink-0">
                      <RiskDial value={resilienceScore} label="Resilience" />
                    </div>
                    <div className="text-xs">
                      <span className="font-semibold text-ink-900 block">Catchment Index</span>
                      <span className="text-ink-500">Vulnerability Score: {currentLocation.vulnerabilityIndex}/100</span>
                    </div>
                  </div>
                </div>
                ...
                {/* Example usage of EmptyState if active alerts list happens to be empty */}
                {activeAlertsList.length === 0 ? (
                  <EmptyState 
                    title="No Active Alerts" 
                    description="All monitored sectors are currently operating within safe baseline thresholds." 
                  />
                ) : null}
