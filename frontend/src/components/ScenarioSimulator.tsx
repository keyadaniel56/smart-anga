    ...
    <div id="scenario-simulator-container" className="space-y-6">
      ...
      <div className="bg-surface-800/80 border border-surface-600 rounded-3xl p-5 sm:p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={handleReset}
            className="px-3.5 py-2 rounded-xl bg-surface-800 hover:bg-surface-600 text-ink-700 text-xs font-semibold transition-all border border-surface-600 flex items-center gap-1.5"
          >
          ...
          <button
            ...
            disabled={isRunning}
            className="px-5 py-2 rounded-xl bg-forest-900 hover:bg-forest-700 text-ink-900 text-xs font-bold transition-all shadow-sm flex items-center gap-2"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {presets.map((p, idx) => (
            <button
              ...
              className="bg-surface-800/80 hover:bg-surface-700 border border-surface-600 hover:border-forest-700/40 p-3.5 rounded-2xl text-left transition-all shadow-xs group"
            >
              <div className="flex items-center justify-between text-xs font-bold text-ink-900 group-hover:text-forest-600">
                <span>{p.name}</span>
                <span className="font-mono text-[10px] text-forest-600">{t('simulator.applyPreset', 'Test This')} &rarr;</span>
              </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-surface-800/80 border border-surface-600 rounded-2xl p-4 shadow-xs space-y-3">
          <div className="flex items-center justify-between text-xs">
            <span className="font-bold text-ink-900">{t('simulator.rainfallChange', 'Rainfall Change')}</span>
            <span className={`font-mono font-bold px-2 py-0.5 rounded-md ${
              precipDelta > 0 ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/20' : 'bg-amber-500/10 text-amber-400 border border-amber-500/20'
            }`}>
          ...
          <input
            ...
            className="w-full accent-teal-600 bg-surface-600 cursor-pointer h-2 rounded-lg"
          />
        ...
        <div className="bg-surface-800/80 border border-surface-600 rounded-2xl p-4 shadow-xs space-y-3">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="font-mono font-bold px-2 py-0.5 rounded-md bg-rose-500/10 text-rose-400 border border-rose-500/20">
          ...
          <input
            ...
            className="w-full accent-rose-600 bg-surface-600 cursor-pointer h-2 rounded-lg"
          />
          <div className="flex justify-between text-[10px] text-ink-400 font-mono">
            <span>+0.5°C</span>
        ...
        <div className="bg-surface-800/80 border border-surface-600 rounded-2xl p-4 shadow-xs space-y-3">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="font-mono font-bold px-2 py-0.5 rounded-md bg-amber-500/10 text-amber-400 border border-amber-500/20">
          ...
          <input
            ...
            className="w-full accent-amber-600 bg-surface-600 cursor-pointer h-2 rounded-lg"
          />
          <div className="flex justify-between text-[10px] text-ink-400 font-mono">
        ...
        <div className="bg-surface-800/80 border border-surface-600 rounded-2xl p-4 shadow-xs space-y-3">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="font-mono font-bold px-2 py-0.5 rounded-md bg-forest-50 text-forest-600 border border-forest-800/30">
          ...
          <input
            ...
            className="w-full accent-forest-700 bg-surface-600 cursor-pointer h-2 rounded-lg"
          />
          <div className="flex justify-between text-[10px] text-ink-400 font-mono">
            <span>0.5x</span>
      ...
      <div className="bg-surface-800/80 border border-surface-600 rounded-3xl p-6 shadow-sm space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-4 border-b border-surface-600">
          <div>
            <div className="flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-surface-800/70 border border-surface-600 rounded-2xl p-4 flex flex-col items-center justify-center text-center shadow-xs">
          ...
          {/* Inundation Area */}
          <div className="bg-surface-800/70 border border-surface-600 rounded-2xl p-4 flex flex-col justify-between shadow-xs">
            ...
            <div className="w-full bg-surface-600 h-1.5 rounded-full overflow-hidden">
              <div
        ...
        <div className="bg-surface-800/80 border border-surface-600 rounded-2xl p-4 space-y-2.5">
          <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-forest-600 font-mono">
            <ShieldCheck className="w-4 h-4 text-forest-600" />
            {t('simulator.mitigationHeading', 'Recommended Actions for This Scenario')}
          </div>
          <div className="space-y-1.5">
            {results.mitigationActions.map((action, i) => (
              <div key={i} className="flex items-start gap-2 text-xs text-ink-700 bg-surface-700 p-2.5 rounded-xl border border-surface-600 shadow-2xs">
